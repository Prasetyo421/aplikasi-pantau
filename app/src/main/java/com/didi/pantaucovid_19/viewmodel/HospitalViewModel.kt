...
class HospitalViewModel : ViewModel() {
    ...
    private val _listHospitalsCovid: MutableLiveData<List<HospitalsCovidItem>> =
        MutableLiveData<List<HospitalsCovidItem>>()
    val listHospitalsCovid: LiveData<List<HospitalsCovidItem>> = _listHospitalsCovid

    private val _listHospitalsNonCovid: MutableLiveData<List<HospitalsNonCovidItem>> =
        MutableLiveData<List<HospitalsNonCovidItem>>()
    val listHospitalsNonCovid: LiveData<List<HospitalsNonCovidItem>> = _listHospitalsNonCovid

    private val _bedDetailHospital: MutableLiveData<List<BedDetailItem>> =
        MutableLiveData<List<BedDetailItem>>()
    ...
    fun setHospitalsData(provId: String, cityId: String, type: String) {
        viewModelScope.launch(Dispatchers.Default) {
            ...
            if (type == TYPE_COVID) {
                client.getCovidHospitals(provId, cityId, TYPE_COVID)
                    .enqueue(object : Callback<ResponseCovidHospitals> {
                        override fun onResponse(
                ...
                client.getNonCovidHospitals(provId, cityId, type)
                    .enqueue(object : Callback<ResponseNonCovidHospitals> {
                        override fun onResponse(
    ...
    fun setBedDetailHospital(hospitalId: String, type: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            Timber.d("current thread:  ${Thread.currentThread().name}")
            client.getDetailHospital(hospitalId, type)
                .enqueue(object : Callback<ResponseDetailHospital> {
                    override fun onResponse(
